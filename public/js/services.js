...
app.factory("foodSelectionService", function($http) {
	...
	function getFruitItems() {
		var fruitItems = [
			{name: "Apple", servings: 1, servSize: "242g", group: "fruit",
	...
	function getProteinItems() {
		var proteinItems = [
			...
			{name: "Eggs", servings: 1, servSize: "100g", group: "proteins",
			...
			{name: "Pork Chop", servings: 1, servSize: "219g", group: "proteins",
	...
	function getVegItems () {
		var vegItems = [
			...
			{name: "Avocado", servings: 1, servSize: "100g", group: "vegetable",
			cal: 160, carbs: 9, protein: 2, fat: 15, meal: ["breakfast", "lunch", "dinner"], img: "imgs/veg/avocado.jpeg"},
	...
	function saveCurrentLog(logday) {
		return $http.put("/api/items/save/" + logday);
	};

	function loadSavedLog(logday) {
		return $http.get("/api/items/" + logday).then(function(response) {
			return response.data;
		});
	};

	function newLog() {
		return $http.get("/api/items/new").then(function(response) {
			return response.data;
		});
	};
